...
@Component({
  selector: 'app-current-wagers',
  templateUrl: './current-wagers.component.html',
  styleUrls: ['./current-wagers.component.scss']
})
export class CurrentWagersComponent extends Extender implements OnInit {
  ...
  private _getRaffleEntries(): void {
    ...
    this.raffleEntryRef.on('change', {
      ...
      include: {
        relation: 'raffleDraw', scope: { where: { isDeleted: false, endDate: { gt: new Date() } } }
      }
